...
quality gates, forma de /predict y concept drift vía chi2.
"""
...
class ClassificationProblem(ProblemType):
    ...
    def quality_gate(
        ...
        if min_accuracy is not None and metrics.get("accuracy", 0.0) < min_accuracy:
            failures.append(f"accuracy {metrics.get('accuracy', 0.0):.4f} < {min_accuracy:.4f}")
        min_f1 = gates_cfg.get("min_f1")
        if min_f1 is not None and metrics.get("f1", 0.0) < min_f1:
            failures.append(f"f1 {metrics.get('f1', 0.0):.4f} < {min_f1:.4f}")
    ...
    def build_predictions(
        ...
        for i, class_id in enumerate(y_pred):
            ...
            entry: Dict[str, Any] = {"class_id": class_id, "class_name": target_names[class_id]}
    ...
    def concept_drift(
        ...
        return bool(p_value < p_value_threshold), {
            "method": "chi2", "statistic": float(statistic), "p_value": float(p_value),
        }
